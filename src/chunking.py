...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        for index in range(0, len(sentences), self.max_sentences_per_chunk):
            chunk_sentences = sentences[index : index + self.max_sentences_per_chunk]
...
class RecursiveChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        return [chunk for chunk in self._split(text, separators) if chunk.strip()]
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        strategies = {
            "fixed_size": FixedSizeChunker(chunk_size=chunk_size, overlap=min(50, max(chunk_size // 5, 0))),
